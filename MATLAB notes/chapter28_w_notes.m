...
First, letÅfs think about our model: We want to test for a linear change (the
...
of A (in fact, itÅfs the same numbers!). So (A'*A) (in math terms, that would
...
n, which has no effect because the transpose of a 1 Å~ 1 matrix is the same
1 Å~ 1 matrix.
%}
A = [ ones(10, 1), (1:n)' ];
